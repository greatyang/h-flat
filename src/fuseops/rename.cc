...
int pok_rename (const char *user_path_from, const char *user_path_to)
{
	...
	if(!err) lookup_parent(user_path_from, dir_mdifrom);
	...
	/* Path does exist. Make sure we're allowed to proceed. */
		...
		if(  fuse_get_context()->uid &&	(dir_mdito->pbuf()->mode() & S_ISVTX) &&
			(fuse_get_context()->uid !=  dir_mdito->pbuf()->uid()) &&
	...
	if(S_ISDIR(mode)){
		...
		if(!err && dir_mdifrom->getSystemPath().compare(dir_mdito->getSystemPath()))
