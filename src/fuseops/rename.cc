...
static int move_directory(const char *user_path_from, const char *user_path_to,
        const std::shared_ptr<MetadataInfo> &mdito,
        const std::shared_ptr<MetadataInfo> &dir_mdito,
        const std::shared_ptr<MetadataInfo> &dir_mdifrom)
{
    ...
    int err = database_operation(
            std::bind(create_metadata, mdito),
            std::bind(delete_metadata, mdito),
            entry);
    if(err)
        return err;
...
int pok_rename(const char *user_path_from, const char *user_path_to)
{
    ...
    if (!err) err = lookup_parent(user_path_from, dir_mdifrom);
